...
export default function Dashboard({ students, teachers, courses, enrollments, onNavigate }: DashboardProps) {
  ...
  const stats = [
    { label: 'Total Students', value: students.length, sub: `${activeStudents} active`, icon: '🎓', color: 'bg-indigo-50 border-indigo-200', iconBg: 'bg-indigo-100', nav: 'students' as ActivePage },
    ...
    { label: 'Enrollments', value: enrollments.length, sub: `${enrolledCount} active`, icon: '📋', color: 'bg-rose-50 border-rose-200', iconBg: 'bg-rose-100', nav: 'enrollments' as ActivePage },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: 'Active Enrollments', value: enrolledCount, color: 'text-indigo-600', bg: 'bg-indigo-50' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <button onClick={() => onNavigate('students')} className="text-indigo-600 text-xs font-semibold hover:underline bg-transparent border-0 cursor-pointer">View all →</button>
          </div>
                ...
                <tr key={s.student_id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-3 text-right">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold
                      ${s.status === 'active' ? 'bg-emerald-100 text-emerald-700' : s.status === 'graduated' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600'}`}>
                      {s.status}
                    </span>
                  </td>
                </tr>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <button onClick={() => onNavigate('enrollments')} className="text-indigo-600 text-xs font-semibold hover:underline bg-transparent border-0 cursor-pointer">View all →</button>
          </div>
          <table className="w-full">
            <tbody>
              {recentEnrollments.length === 0 ? (
                ...
                return (
                  <tr key={e.enrollment_id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-right">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold
                        ${e.status === 'enrolled' ? 'bg-indigo-100 text-indigo-700'
